...
export default class SpeciesResource extends Resource {
  ...
  readonly pokedex_numbers: Array<{ entry_number: number, pokedex: PokedexEntryType }> = []
  readonly genera: Array<{ genus: string, language: LanguageType }> = []
  readonly varieties: Array<{ is_default: boolean, pokemon: NamedApiResource }> = []
  ...
  getEntryNumber(type = 'national') {
    return this.pokedex_numbers.find(entry => entry.pokedex.name === type)?.entry_number
  }
  ...
  static listShape<T extends typeof Resource>(this: T) {
    return {
      ...super.listShape(),
      schema: { results: [this.getEntitySchema()], count: 0, next: '', previous: '' },
    };
  }
