...
import { Card, CardContent, CardTitle, CardSubtitle, CardMedia, CardSection } from '../ui'
...
const Species: React.FC<SpeciesProps> = ({ name }) => {
  ...
  const variety = useResource(VarietyResource.detailShape(), { name: species.defaultVariety.name })
  ...
  return (
    <Card>
      <CardContent>
        ...
        <CardTitle>{species.getEntryNumber()} - {species.getName()}</CardTitle>
        <CardSubtitle>{species.getGenus()}</CardSubtitle>
        <CardSection>
          {variety.getTypes().map(t => <Type key={`${variety.name}-${t.name}`} name={t.name} />)}
        </CardSection>
      </CardContent>
    </Card>
  )
